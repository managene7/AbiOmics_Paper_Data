...
def Inference(model, data, target):
    ...
    with torch.no_grad():  
        ...
        output = model(x)
        y = torch.Tensor([to_one_hot(label,5) for label in y.cpu().tolist()]).to(device)
...
def Accuracy(output, target):
    ...
    labels = target.cpu()
    target_names = ['Control', 'Salt', 'Cold', 'Heat','Drought']
    return_values = [confusion_matrix(labels, y_hat), classification_report(labels, y_hat, target_names=target_names)]
...
def training(x_data, y_data, min_max_dic, all_skfold, 
    ...
    for fold, (idx_train, idx_val_test) in enumerate(all_skfold.split(x_data, y_data)):
        ...
        model = mlp_model(dim1=input_dim, label_dim=5).to(device)
        ...
        for epoch in tqdm(range(N_EPOCH), desc=f"Cross Validation Fold: {fold + 1}", ncols=200):
            ...
            val_accuracy = Accuracy(val_output, val_target)
        ...
        if early_stop == True:
            ...
            test_result_list.append((fold+1, val_acc_final, val_loss_final, test_accuracy, test_loss)) #test, train,val ACC, test accuracy
            
            label_dic={0: 'Control', 1: 'Salt', 2: 'Cold', 3: 'Heat', 4: 'Drought'}
            print("Validation Confusion Matrix:\n")
            plot_confusion(val_accuracy[0], label_dic)
